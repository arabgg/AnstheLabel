...
<div class="p-8 bg-white rounded-lg shadow">
    ...
    <div class="flex justify-end mb-7 mt-12">
        <form method="GET" action="{{ route('bahan.index') }}" class="mr-3 flex items-center border rounded-lg px-3 py-2 w-1/3">
            <input type="text" name="search" placeholder="Search Bahan" value="{{ $searchQuery ?? '' }}" class="w-full outline-none placeholder:text-sm">
            <button type="submit" class="ml-2">
                <i class="fas fa-search"></i>
            </button>
        </form>
        
        <a href="{{ url('/bahan/create') }}"
    ...
    <div class="mt-4 flex justify-center space-x-1">
        @if ($bahan->onFirstPage() === false)
            <a href="{{ $bahan->previousPageUrl() }}" class="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">&laquo;</a>
        @endif
