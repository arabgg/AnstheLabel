...
<div class="p-8 bg-white rounded-lg shadow">
    ...
    <div class="flex justify-end mb-5">
        <form method="GET" action="{{ route('banner.index') }}" class="flex items-center border rounded px-3 py-2 w-1/3">
            <input type="text" name="search" placeholder="Search Banner" value="{{ $searchQuery ?? '' }}" class="w-full outline-none placeholder:text-sm">
            ...
            <tbody class="text-sm">
                @forelse ($banners as $banner)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="p-3">{{ $banner->banner_id }}</td>
    ...
    <div class="mt-4 flex justify-center space-x-1">
        @if ($banners->onFirstPage() === false)
            <a href="{{ $banners->previousPageUrl() }}" class="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">&laquo;</a>
        @endif
        ...
        @if ($banners->hasMorePages())
            <a href="{{ $banners->nextPageUrl() }}" class="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">&raquo;</a>
        @endif
    </div>
</div>
