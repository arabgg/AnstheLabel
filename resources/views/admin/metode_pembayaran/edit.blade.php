<div class="bg-white rounded-lg shadow p-6">
    ...
    <form action="{{ route('metode_pembayaran.update', $metode->metode_pembayaran_id) }}" method="POST"
        id="editMetodeForm" enctype="multipart/form-data" class="text-sm">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nama_pembayaran" value="{{ $metode->nama_pembayaran }}"
                class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#560024]">
        </div>

        {{-- Kode Bayar --}}
        <div class="mb-4" id="kode_bayar_text"
            style="{{ $metode->kode_bayar_type === 'text' ? '' : 'display:none;' }}">
            <label class="block text-sm font-medium mb-1">Nomor Rekening / E-Wallet</label>
            <input type="text" name="kode_bayar"
                value="{{ $metode->kode_bayar_type === 'text' ? $metode->kode_bayar : '' }}"
                class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#560024]">
        </div>
