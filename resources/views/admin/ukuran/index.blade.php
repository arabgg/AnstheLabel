...
<div class="p-8 bg-white rounded-lg shadow">
    ...
    <div class="flex justify-end mb-7 mt-12">
        <form method="GET" action="{{ route('ukuran.index') }}" class="mr-3 flex items-center border rounded-lg px-3 py-2 w-1/3">
            <input type="text" name="search" placeholder="Search Ukuran" value="{{ $searchQuery ?? '' }}" class="w-full outline-none placeholder:text-sm">
            <button type="submit" class="ml-2">
                <i class="fas fa-search"></i>
            </button>
        </form>
        
        <a href="{{ url('/ukuran/create') }}"
            ...
            <tbody class="text-sm">
                @forelse ($ukuran as $item)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="p-3">{{ $item->ukuran_id }}</td>
    ...
    <div class="mt-4 flex justify-center space-x-1">
        @if ($ukuran->onFirstPage() === false)
            <a href="{{ $ukuran->previousPageUrl() }}" class="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">&laquo;</a>
        @endif
                    ...
                    form.addEventListener('submit', function(e){
                        ...
                        .then(data => {
                            ...
                            if(data.success){
                                ...
                                const row = document.querySelector(`[data-ukuran-id='${data.data.ukuran_id}']`);
                                if(row){
