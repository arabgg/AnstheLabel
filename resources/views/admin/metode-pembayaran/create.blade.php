...
<div class="max-w-3xl mx-auto p-6">
    ...
    <form action="{{ route('metode-pembayaran.store') }}" method="POST" class="space-y-4">
        @csrf

        <div>
            <label class="block font-medium">Nama Pembayaran</label>
            <input type="text" name="nama_pembayaran" value="{{ old('nama_pembayaran') }}" class="w-full border rounded px-3 py-2">
            @error('nama_pembayaran') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block font-medium">Kode Bayar</label>
            <input type="text" name="kode_bayar" value="{{ old('kode_bayar') }}" class="w-full border rounded px-3 py-2">
        ...
        <div>
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded shadow hover:bg-blue-600">Simpan</button>
